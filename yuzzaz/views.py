...
def homepage(request):
    ...
    today = timezone.localdate()  # safer for comparing with DateFields
    date_obj = settings.regular_consideration_deadline
    if isinstance(date_obj, datetime):
        js_final_deadline = localtime(date_obj).isoformat()
    else:
        js_final_deadline = make_aware(datetime.combine(date_obj, time(23, 59, 59))).isoformat()

    portal_open = ApplicationSettings.get_state().is_portal_open
    # portal_open = (
    #     settings.is_portal_open or (
    #         settings.start_date and
    #         settings.regular_consideration_deadline and
    #         settings.start_date <= today <= settings.regular_consideration_deadline
    #     )
    # )

    return render(request, 'yuzzaz/homepage.html', {
        'portal_open': portal_open,
        'final_deadline': settings.regular_consideration_deadline,
        'js_final_deadline': js_final_deadline,
    })
...
def portal_settings_view(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        messages.success(request, "Portal has been updated successfully")
        return redirect('intern_view_all_applications')
