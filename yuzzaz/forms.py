...
class ApplicationSettingsForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'is_portal_open',
            'start_date',
            'early_consideration_deadline',
            'regular_consideration_deadline',
        ]
        widgets = {
            'is_portal_open': forms.CheckboxInput(attrs={'class': 'form-checkbox'}),
            'start_date': forms.DateInput(attrs={'type': 'date', 'class': 'form-input'}),
            'early_consideration_deadline': forms.DateInput(attrs={'type': 'date', 'class': 'form-input'}),
            'regular_consideration_deadline': forms.DateInput(attrs={'type': 'date', 'class': 'form-input'}),
        }

    def clean(self):
        cleaned_data = super().clean()
        start = cleaned_data.get('start_date')
        early = cleaned_data.get('early_consideration_deadline')
        regular = cleaned_data.get('regular_consideration_deadline')

        if start and early and regular:
            if not (start < early < regular):
                raise ValidationError("Ensure: start_date < early_consideration_deadline < regular_consideration_deadline")
